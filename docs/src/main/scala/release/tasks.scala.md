...
```scala
  def prepareRelease(releaseVersion: Version): DefTask[Unit] = Def.sequential(
    ...
    announce("Generating sources documentation..."),
    commitLiteratorDocs(releaseVersion),
...
Cleans previously generated docs, generates them again and commits

```scala
  def commitLiteratorDocs(releaseVersion: Version): DefTask[Unit] = Def.taskDyn {

    val outputs = docsOutputDirs.value
    ...
    Def.task {
      ...
      if (! git.isDirty(withUntracked = true)) {
        ...
        git.stageAndCommit(s"Autogenerated literator docs for v${releaseVersion}")(outputs: _*)
      }
    }
  }
```
